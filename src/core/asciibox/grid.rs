// using https://github.com/becheran/grid
use std::{
    cmp, fmt, hash,
    iter::StepBy,
    ops::{Index, IndexMut},
...
macro_rules! grid {
    () => {
        $crate::core::asciibox::grid::Grid::from_vec(vec![], 0)
    };
    ( [$( $x:expr ),* ]) => { {
        let vec = vec![$($x),*];
        let len  = vec.len();
        $crate::core::asciibox::grid::Grid::from_vec(vec, len)
    } };
    ( [$( $x0:expr ),*] $([$( $x:expr ),*])* ) => {
        {
            ...
            $crate::core::asciibox::grid::Grid::from_vec(vec, cols)
        }
    };
}
...
macro_rules! grid_cm {
    () => {
       $crate::core::asciibox::grid::Grid::from_vec_with_order(vec![], 0, $crate::core::asciibox::grid::Order::ColumnMajor)
    };
    ( [$( $x:expr ),* ]) => { {
        let vec = vec![$($x),*];
        let len  = vec.len();
        $crate::core::asciibox::grid::Grid::from_vec_with_order(vec, len, $crate::core::asciibox::grid::Order::ColumnMajor)
    } };
    ( [$( $x0:expr ),*] $([$( $x:expr ),*])* ) => {
        {
            let mut _assert_width0 = [(); $crate::count!($($x0)*)];
            ...
            let mut grid = $crate::core::asciibox::grid::Grid::from_vec_with_order(vec, cols, $crate::core::asciibox::grid::Order::ColumnMajor);
...
impl<T: Default> Grid<T> {
    ...
    /// Expands the grid by inserting a given amount of rows at the beginning, filling the new rows with `T::default()`.
    /// If the grid has no rows or no columns, nothing will happen.
    ///
    /// # Examples
    ///
    ...
    /// Expands the grid by inserting a given amount of columns at the beginning, filling the new columns with `T::default()`.
    /// If the grid has no rows or no columns, nothing will happen.
    ///
    /// # Examples
    ///
            ...
            match self.order {
                ...
                Order::ColumnMajor => {
                    for i in (0..self.cols).rev() {
                        ...
                        self.data[s..e].rotate_left(self.rows);
                    }
                }
            }
            self.cols += cols;
...
mod test {
    ...
    fn prepend_rows() {
        ...
        assert_eq!(grid.rows(), 4);
        assert_eq!(grid.cols(), 3);
        assert_eq!(grid.into_vec(), vec![0, 0, 0, 0, 0, 0, 1, 1, 1, 2, 2, 2]);
    }
    ...
    fn prepend_rows_column_major() {
        ...
        assert_eq!(grid.rows(), 4);
        assert_eq!(grid.cols(), 3);
        assert_eq!(grid.into_vec(), vec![0, 0, 1, 2, 0, 0, 1, 2, 0, 0, 1, 2]);
    }
    ...
    fn prepend_cols() {
        ...
        assert_eq!(grid.rows(), 2);
        assert_eq!(grid.cols(), 5);
        assert_eq!(grid.into_vec(), vec![0, 0, 1, 1, 1, 0, 0, 2, 2, 2]);
    }
    ...
    fn prepend_cols_column_major() {
        ...
        assert_eq!(grid.rows(), 2);
        assert_eq!(grid.cols(), 5);
        assert_eq!(grid.into_vec(), vec![0, 0, 0, 0, 1, 2, 1, 2, 1, 2]);
    }
    ...
    fn iter_rows() {
        ...
        let max_by_row: Vec<u8> = grid
            .iter_rows()
            .map(|row: StepBy<Iter<'_, u8>>| row.max().unwrap())
            .copied()
            .collect();
        assert_eq!(max_by_row, vec![3, 6]);

        let sum_by_row: Vec<u8> = grid
            .iter_rows()
            .map(|row: StepBy<Iter<'_, u8>>| row.sum())
            .collect();
        assert_eq!(sum_by_row, vec![1 + 2 + 3, 4 + 5 + 6]);
    }
    ...
    fn iter_cols() {
        ...
        let max_by_col: Vec<u8> = grid
            .iter_cols()
            .map(|col: StepBy<Iter<'_, u8>>| col.max().unwrap())
            .copied()
            .collect();

        assert_eq!(max_by_col, vec![4, 5, 6]);

        let sum_by_col: Vec<u8> = grid
            .iter_cols()
            .map(|row: StepBy<Iter<'_, u8>>| row.sum())
            .collect();
        assert_eq!(sum_by_col, vec![1 + 4, 2 + 5, 3 + 6]);
